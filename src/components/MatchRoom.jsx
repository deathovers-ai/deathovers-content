  ...
  return (
    <section className="panel live-panel board-enter">
      ...
      <div className="context-rail">
        <ContextCell label="Toss" value={view.tossText} accent={view.tossText && view.tossText !== 'Awaiting toss' ? 'amber' : ''} />
      ...
      {view.dewRisk?.model_note && (
        <p className="source-note dew-model-note">Model: {view.dewRisk.model_note}</p>
      )}
...
function buildView(data) {
  ...
  return {
    ...
    tossText: data?.tossLine
      || (data?.toss?.winner
        ? `${data.toss.winner} won the toss, elected to ${data.toss.decision === 'bat' ? 'bat' : data.toss.decision === 'bowl' ? 'bowl' : 'play'}`
        : (typeof data?.toss === 'string' && data.toss.trim()) || 'Awaiting toss'),
    ...
    dewText: data?.dewRisk?.risk
      ? `${data.dewRisk.risk} risk${data.dewRisk.model_applied ? ' · model on' : ''}`
      : 'No alert',
