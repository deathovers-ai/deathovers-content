                  ...
                  {activeTab === 'bowling' && (
                    <div className="stats-table font-mono">
                      ...
                      {(selectedInnings === 1 ? mockMatchDetails.innings1.bowlers : mockMatchDetails.innings2.bowlers).map((b, i) => (
                        <div key={i} className="table-row bowler-row"><div className="bowler-main"><span className="player-name">{b.name}</span><span>{b.overs}-{b.maidens}-{b.runs}-{b.wickets}</span><span className="txt-amber">{b.econ}</span></div><div className="spell-highlight">{b.currentSpell}</div></div>
                      ))}
                    </div>
                  )}
                  ...
                  {activeTab === 'commentary' && (
                    <div className="commentary-list font-mono">
                      {mockMatchDetails.commentary.map((c, i) => (
                        <div key={i} className="comm-row"><div className="comm-meta"><span className="comm-over">{c.over}</span><span className={`comm-badge ${c.event === 'SIX' || c.event === 'WICKET' ? 'alert' : ''}`}>{c.event}</span></div><div className="comm-text">{c.desc}</div></div>
                      ))}
                    </div>
                  )}
                </div>
