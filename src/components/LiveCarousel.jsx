...
export default function LiveCarousel() {
  ...
  const fetchLiveCluster = async () => {
    // NEW: abort the request after 12s instead of letting it hang indefinitely â€”
    ...
    try {
      ...
      // Only show the error state if we have no data at all yet â€” if we're on a
  ...
  // exactly once on open and never again â€” so anyone sitting on a live match page
  ...
  // supposed to be live â€” surfaces the staleness instead of silently showing
        ...
        <div className="carousel-wrap">
          <div className="carousel-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 24px', marginBottom: '10px' }}>
            ...
            <div className="carousel-controls" style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => scrollByCard(-1)} className="carousel-btn">â€¹</button>
              <button onClick={() => scrollByCard(1)} className="carousel-btn">â€º</button>
            </div>
          </div>
          ...
          >
            {displayMatches.map((match) => {
              ...
              {/* FIX: use the clean teams[] array from the backend instead of splitting
                  matchName on " vs " â€” matchName often has a trailing ", 3rd Match" etc.
              ...
              <div className="scoreboard">
                ...
                {isDataStale && (
                  <div className="stale-banner">
                    Scorecard may be a few minutes behind â€” refreshing shortly.
                  </div>
                )}
                  ...
                  <div className="ball-tracker">
                    ...
                    <div className="ball-tracker-dots">
                      ...
                      {Array.from({ length: Math.max(0, 6 - ballTracker.length) }).map((_, i) => (
                        <span key={`empt…2287 tokens truncated…em; color: var(--crease-white);
        }
        .team-code-text {
          overflow: hidden;
  ...
  return (
    <div className="innings-col border-left">
      <div className={`inn-heading accent-${accent}`}>
        {label}: {innings.team || 'TBD'} Â· {innings.score || '0/0'} ({innings.overs || '0.0'})
      </div>
  ...
  return (
    <div className="live-engine-wrapper">
        ...
        <div className="matchpage">
          ...
          <>
              ...
              <div className="scoreboard">
                ...
                {/* One combined CTA — tactical reads + match room live in a
                    separate window so scoreboard + commentary keep the space. */}
                <a
                  href={`/match-room?id=${activeMatchId}`}
                  className="match-intel-cta"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <span className="match-intel-cta-copy">
                    <span className="match-intel-cta-label">TACTICAL READS · MATCH ROOM</span>
                    <span className="match-intel-cta-sub">Open stats &amp; venue intel in a new window</span>
                  </span>
                  <span className="match-intel-cta-arrow" aria-hidden="true">↗</span>
                </a>
              </div>
      ...
      <style>{`
        ...
        .match-intel-cta:hover { background: rgba(245,166,35,0.12); }
        .match-intel-cta-copy { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
        .match-intel-cta-label {
          font-family: 'JetBrains Mono', monospace;
          font-size: 11px;
          font-weight: 700;
