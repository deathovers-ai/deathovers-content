...
function VenueRecord({ brief, toss, weather, dewRisk }) {
  ...
  return <section className="tab-panel venue-panel" role="tabpanel">
    ...
    <div className="conditions-grid">
      <Info label="This match · Toss" value={tossLine || 'Awaiting toss announcement'} tone={tossLine ? 'live' : ''} />
      ...
      <Info label="Dew" value={dewRisk?.risk ? `${dewRisk.risk} risk${dewRisk.model_applied ? ' · model on' : ''}` : 'No dew alert'} tone={dewRisk?.risk === 'HIGH' ? 'risk' : ''} />
      {dewRisk?.model_note ? <p className="weather-model-note">{dewRisk.model_note}</p> : null}
    </div>
    {tossLine && (
      <div className="toss-result-banner">
        <span>TOSS RESULT</span>
        <strong>{tossLine}</strong>
      </div>
    )}
...
function formatToss(toss) {
  ...
  if (toss.winner) {
    ...
    const verb = decision === 'bat' ? 'elected to bat' : decision === 'bowl' ? 'elected to bowl' : 'won the toss';
    return decision === 'bat' || decision === 'bowl'
      ? `${toss.winner} won the toss, ${verb}`
